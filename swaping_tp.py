...
def swappingplt():
    ...
    if jump == 0:
        ...
        for item in dataset:
            ...
            ax.bar(bin_edge[:-1], hist/hist[0],color=cmap[int((ind+colorshift)*colorint)],align = 'edge', width=width[ind], edgecolor = 'k', zorder=-ind,alpha=0.8)
            # Curve_fit method
            ...
            ax.plot(xt, ytcum / ytcum[0], '--', zorder=ind, linewidth=2, label=legend[ind], color=cmap[int((ind+colorshift)*colorint)])
            if item =='ecc095':
        ...
        x = [0, 1, 2, 3,4,5,6]
        # y = [0, 2, 4]
        ax.errorbar(x, ts, yerr=es, linestyle='None', marker='.', ms=13, capsize=8, color='r', label='Short time-scale')
        ax.errorbar(x, tl, yerr=el, linestyle='None', marker='.', ms=13, capsize=8, color='k', label='Long time-scale')
        ax.set_xlabel('Cavity eccentricity', fontsize=15)
        ax.set_ylabel('Relaxation time (s)', fontsize=15)
