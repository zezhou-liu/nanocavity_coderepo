import walldepletionfitting_wca as wf
import module
import os
import numpy as np
...
def simu_generate():
    ...
    sim_comparepath = '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/simulationdata_wca/'
    ...
    for folder in foldername:
        if 'ecc' in folder:
            ...
            os.chdir(sim_resultpath+folder+'/wca')
    ...
    xred, yred, prob = wf.wall_deplt_intver_wca(sol[2,:]) # Modify from here to return(line40-43). Also mody walldepletionfitting.py file line451 to line 453
...
def wall_generate():
    # Generate simulation data in folder /home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata/simulationdata/
    sim_resultpath = '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/'
    sim_comparepath = '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/wallprofile_wca/'
    os.chdir(sim_resultpath)
    foldername = os.listdir()
    ...
    sol = np.vstack(sol) # solution with the same order as ecclib
    os.chdir(sim_comparepath)
    x,y,u,prob = wf.potential_generation_plotuseonly_wca(sol[2, 1]) # Modify the FIRST number here for different ecc. Also mody walldepletionfitting_wca.py file line598 to line 603
...
def longprojection_exp_simu_samegraph():
    global exppath
    simupath = '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/simulationdata_wca/'
    ...
    for file in filename:
        if 'ecc' in file:
            ...
            if name[1] not in order:
                ...
                order.append(name[1])
                if name[1] == 'ecc0':
                    xredtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/TOTPOTENTIAL/WCA/ecc0/x_add.txt'))
                    yredtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/TOTPOTENTIAL/WCA/ecc0/y_add.txt'))
                    probtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/TOTPOTENTIAL/WCA/ecc0/landscape_ecc0.txt'))
                elif name[1] == 'ecc0995':
                    xredtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/simulationdata_wca/ecc0995x.txt'))
                    yredtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/simulationdata_wca/ecc0995y.txt'))
                    probtot.append(np.loadtxt(
                        '/home/zezhou/McGillResearch/2019Manuscript_Analysis/femsimulation/iterationdata_sq/simulationdata_wca/ecc0995_prob.txt'))
                else:
                    xredtot.append(np.loadtxt(name[0] + '_' + name[1] + '_' + 'xred.txt'))
                    yredtot.append(np.loadtxt(name[0] + '_' + name[1] + '_' + 'yred.txt'))
                    probtot.append(np.loadtxt(name[0] + '_' + name[1] + '_' + 'prob.txt'))
...
def plot_fig5_bcde():
    ...
    for i in xred:
        if i != tmp:
            ...
            w.append(np.mean(wall[tmask]))
            u.append(np.mean(u0[tmask]))
    w[0]=w[1]*100 # data overflow. Coming from the big number saving issue. Not affecting any thing.
    w[-1] = w[0]
    ...
    for i in yred:
        if i != tmp:
            ...
            w.append(np.mean(wall[tmask]))
            u.append(np.mean(u0[tmask]))
    w[0] = w[1]*100 # Overflow in saving big number in txt file. Not affecting anything
    w[-1] = w[0]
...
longprojection_exp_simu_samegraph()